...
def process(i):
    ...
    obj_filename = f'grasp_data/meshes/{category}/{category}{i:03}.stl'    
    ...
    if len(random_candidates) == 0:
        candidate_indices = random_candidates
    ...
    for k, idx in enumerate(candidate_indices):
        ...
        new_transforms, new_origins, new_quaternions, new_qualities = graspsampler.perturb_grasp_locally(number_of_grasps=NUM_LOCAL_GRASPS,
                                                                            ...
                                                                            beta=beta[idx],
                                                                            gamma=gamma[idx],
                                                                            alpha_range=np.pi/18,
                                                                            beta_range=np.pi/12,
                                                                            gamma_range=np.pi/12,
                                                                            t_range=0.1,
                                                                            silent=True)
