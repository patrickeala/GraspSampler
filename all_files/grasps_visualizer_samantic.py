...
for idx in range(21):
    ...
    grasp_filename = f"semantic_dataset/{obj_class}/{obj_name}/all_stable_grasps_labelled.npz"
    grasp_data = np.load(grasp_filename)
    translations_init = grasp_data["translations"]
    quaternions_init = grasp_data["quaternions"]
    semantic_labels = grasp_data[task]
    print(semantic_labels.shape)
    # print(sum(grasp_data["hammer"]),sum(grasp_data["handover"]))
    # obj_pose_relative = grasp_data["obj_pose_relative"]
    translations_semantic = translations_init[semantic_labels==1]
    quaternions_semantic = quaternions_init[semantic_labels==1]
    pc_filename = f"grasp_data/pcls/{obj_class}/{obj_name}.pkl"
    pc_real = np.array(pickle.load(open(pc_filename, 'rb')))

    # index = np.random.choice(translations_semantic.shape[0], 500, replace=False)  

    # translations_init = translations_init[index]
    # quaternions_init = quaternions_init[index]
    translations_semantic_down_sampled = translations_semantic[:]
    quaternions_semantic_down_sampled = quaternions_semantic[:]
    ...
    for i in range(min(translations_semantic_down_sampled.shape[0],1000)):

        r = R.from_quat(quaternions_semantic_down_sampled[i])
        transform = np.eye(4)
        transform[:3,:3] = r.as_matrix()
        transform[:3,3] = translations_semantic_down_sampled[i]
