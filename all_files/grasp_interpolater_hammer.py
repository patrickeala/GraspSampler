...
def interpolate_grasps(target_grasp, number_grasps):
    ...
    for i in range(number_grasps):
        ...
        grasp[2,3] = target_grasp[2,3] + 0.012 * i + random.random()*0.002
        grasp[1,3] = target_grasp[1,3] - 0.023 * i + random.random()*0.002
        rets.append(grasp)
    return rets
distance = 0.9
transform = np.eye(4)
...
target_grasp = np.array([[-0.5      ,  0   ,   0.8660254, -0.08   ],
 [ 0.       ,  1.      ,   0.        , -0.03     ],
...
for i in range(len(grasps)):
    ...
    # scene.remove_gripper(gripper)
    png = scene.save_image(resolution=[640*2, 480*2],
                        visible=True)
    with open(f"hammer_{15-i}", 'wb') as f:
        f.write(png)
        f.close()
