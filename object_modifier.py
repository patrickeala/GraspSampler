...
cat = "box"
idx = 17
obj_filename = f"grasp_data/meshes/{cat}/{cat}{idx:03}.stl"
...
init_translation = np.array([0.4,0.0,0.0])
init_orientation = np.array([0.0,0.0,0.0])
transform = get_transform(init_translation,init_orientation)

sampler = GraspSampler()
# load object
sampler.update_object(obj_filename=obj_filename, name=obj_filename, obj_scale=[1.0 ,0.4, 0.9])
object2 = Object(filename="assets/meshes_10_objects/004_sugar_box/google_16k/textured.obj", name=1, scale=1)
sampler.scene.add_object(object2)
# sampler.obj.apply_transform(transform)
change = False
