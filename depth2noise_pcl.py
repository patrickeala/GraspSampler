...
if __name__ == "__main__":
    ...
    # depth_map = depth_maps[0]
        # noise_depth = pcl_manager.add_noise_to_depth(depth_map)
        # test_pc = pcl_manager.depth_to_pointcloud(depth_map)
        test_pc = depth2noise_pcl(depth_map,transfered_pose,dropout=0)
        pc = np.concatenate([pc,test_pc])
    
    ori_obj = trimesh.load("assets/meshes_10_objects/004_sugar_box/google_16k/textured.obj")

    pc1 = trimesh.points.PointCloud(vertices=pc)
    pc2 = trimesh.points.PointCloud(vertices=ori_obj.vertices)
    transform = np.eye(4)
    transform[0,3] = 0.3
