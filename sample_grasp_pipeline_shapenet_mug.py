...
def sample_grasps(filename,path,name,scale,path_grasps,path_pcs):
    ...
    # with open(f'{path_grasps}/{filename}.pkl', 'wb') as fp:
...
grasp_save_dir = 'temp_grasps_2/shapenet_grasps'
pc_save_dir = 'temp_grasps_2/shapenet_pcs'

def process(i):
    path_grasps = os.path.join(grasp_save_dir,object)
    path_pcs = os.path.join(pc_save_dir,object)
    filename = f'{data_dir}/{object}/{object}{i:03}.json'
    with open(filename) as json_file:
        data = json.load(json_file)
    name = data['id']
    path = data['path']
    scale = data["scale"]
    target_filename = f"{object}{i:03}"
    sample_grasps(target_filename,path,name,scale,path_grasps,path_pcs)
